...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchUserName = async () => {
      try {
        const response = await fetch("http://localhost:5000/last_logged_in");
        if (!response.ok) {
          throw new Error("Failed to fetch user data");
        }
        const data = await response.json();
        setFullName(data.fullName);
        console.log("User Full Name:", data.fullName);
      } catch (error) {
  ...
  useEffect(() => {
    if (selectedImage) {
      ...
      fetch("http://localhost:5000/image_explain", {
        method: "POST",
        body: formData,
      })
      ...
      <main className="flex-1 overflow-auto">
        <div className="container mx-auto px-4 py-6">
          <div className="mb-6 flex items-center justify-between">
           <div className="flex items-center">
             <h1 className="text-2xl font-bold text-teal-800">Welcome back {fullName}!</h1>
          ...
          {analysisResult && (
            <div className="mt-4">
              ...
              <p className="text-sm text-gray-600">
                The image analysis is not perfect and may not be accurate.
              </p>
              ...
              {analysisResult.sugar && (
                <p>Sugar: {analysisResult.sugar}</p>
              )}
              {analysisResult.water && (
                <p>Water: {analysisResult.water}</p>
              )}
            </div>
          )}
          {/* Sensor Sync Status */}
